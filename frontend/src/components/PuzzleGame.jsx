...
export default function PuzzleGame() {
  ...
  /**
   * Handle Region Capture from react-screen-capture
   */
  const handleRegionCapture = (capture) => {
    setRegionCapture(capture);
  };

  /**
   * Download the region capture
   */
  const handleDownloadRegion = () => {
    const link = document.createElement('a');
    link.download = `region-capture-${Date.now()}.png`;
    link.href = regionCapture;
    link.click();
    setRegionCapture(''); // Clear after download
  };
  ...
  return (
    <ScreenCapture onEndCapture={handleRegionCapture}>
      {({ onStartCapture }) => (
        <div ref={gameRef} className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-100 to-blue-100 py-12 px-4">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <button
                      ...
                      className="px-6 py-2 bg-red-500 text-white rounded-full font-semibold hover:bg-red-600 transition-all shadow-md hover:shadow-lg text-sm"
                    >
                      🔄 Restart
                    </button>
                    {/* react-screen-capture trigger */}
                    <button
                      onClick={onStartCapture}
                      className="px-6 py-2 bg-blue-600 text-white rounded-full font-semibold hover:bg-blue-700 transition-all shadow-md hover:shadow-lg text-sm flex items-center gap-1 justify-center"
                    >
                      ✂️ Select Area
                    </button>
                  </div>
          ...
          {/* Region Capture Preview Modal */}
          {regionCapture && (
            <div className="fixed inset-0 z-[100] bg-black/80 flex flex-col items-center justify-center p-4 backdrop-blur-sm">
              <div className="bg-white p-6 rounded-2xl shadow-2xl max-w-2xl w-full animate-in zoom-in duration-300">
                <h3 className="text-2xl font-bold text-gray-800 mb-4 flex items-center justify-between">
                  🖼️ Region Captured
                  <button onClick={() => setRegionCapture('')} className="text-gray-400 hover:text-red-500">✕</button>
                </h3>
                <img src={regionCapture} alt="Region" className="w-full h-auto rounded-lg border-2 border-gray-100 shadow-inner mb-6" />
                <div className="flex gap-4">
                  <button
